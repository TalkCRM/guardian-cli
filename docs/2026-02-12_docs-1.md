...
- 주기: 분기 1회
- 실행 주기: 분기 2주차 화요일
- 사전 공지: 2주 전 공지, 1주 전 확정, 1일 전 리마인드
- 환경: 스테이징 또는 격리망
- 산출물: 요약 보고 + 상세 보고 + 증거 패키지

### 3.5 AWS 기반 증거 보관 (기본값)

- 증거 저장소: `com.sungmin.networks.talkcrm` (SSE-KMS 암호화, prefix: `compliance-evidence/`)
- 감사 로그:
  - CloudTrail 로그 버킷: `aws-cloudtrail-logs-907056532170-b89cd8f2`
  - CloudWatch Logs (ap-northeast-2)
- 접근 통제: 읽기 전용 감사 역할 + 최소 권한 IAM 정책
- 무결성: 해시(manifest) + 서명 저장
