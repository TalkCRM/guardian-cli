...
# 증거 저장 버킷 (예시)
export EVIDENCE_BUCKET="talkcrm-security-evidence"
export RUN_ID="run-2026-02-20"
```
...
aws s3 cp cloudtrail-events.json \
  "s3://${EVIDENCE_BUCKET}/${RUN_ID}/logs/cloudtrail-events.json"
```

#### 3.4.2 CloudWatch Logs 내보내기

```bash
export LOG_GROUP="/aws/llm-app/audit"
...
aws s3 cp cloudwatch-audit.json \
  "s3://${EVIDENCE_BUCKET}/${RUN_ID}/logs/cloudwatch-audit.json"
```
...
aws s3 cp manifest.json "s3://${EVIDENCE_BUCKET}/${RUN_ID}/manifest.json"
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:s3:::talkcrm-security-evidence",
        "arn:aws:s3:::talkcrm-security-evidence/*"
      ]
    }
  ]
